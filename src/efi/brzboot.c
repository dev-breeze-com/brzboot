 ...
 * menu.
 *
 * All brzboot code is LGPL not GPL, to stay out of politics and to give
 ...
 *
 * Copyright (C) 2019 Pierre Innocent <dev@breezeos.com>
...
static const char __attribute__((used)) magic[] = "#### LoaderInfo: brzboot " VERSION " ####";
...
static VOID print_status(Config *config, EFI_FILE *root_dir, CHAR16 *loaded_image_path) {
    ...
    for (;;) {
        ...
        if (EFI_ERROR(err))
            err = graphics_splash(root_dir, L"\\EFI\\brzboot\\splash.bmp", pixel);
        if (EFI_ERROR(err))
            break;
    ...
    Print(L"brzboot version:      " VERSION "\n");
    Print(L"brzboot architecture: " MACHINE_TYPE_NAME "\n");
...
static BOOLEAN menu_run(Config *config, ConfigEntry **chosen_entry, EFI_FILE *root_dir, CHAR16 *loaded_image_path) {
    ...
    while (!exit) {
        ...
        case KEYPRESS(0, 0, 'v'):
            status = PoolPrint(L"brzboot " VERSION " (" MACHINE_TYPE_NAME "), UEFI Specification %d.%02d, Vendor %s %d.%02d",
                               ST->Hdr.Revision >> 16, ST->Hdr.Revision & 0xffff,
...
static BOOLEAN config_entry_check_validity(ConfigEntry *entry, EFI_HANDLE *device, CHAR16 *initrd, CHAR16 *file) {
...
static VOID config_entry_add_from_file(Config *config, EFI_HANDLE *device, CHAR16 *file, CHAR8 *content, CHAR16 *loaded_image_path) {
    ...
    while ((line = line_get_key_value(content, (CHAR8 *)" \t", &pos, &key, &value))) {
        ...
        if (strcmpa((CHAR8 *)"title", key) == 0) {
			...
			if (title && !entry->title) {
				...
				if (config_entry_check_validity( entry, device, initrd, NULL ))
					config_add_entry(config, entry);
            	FreePool(entry->title);
				entry->title = NULL;
			}
			title = stra_to_str(value);
        }
        ...
        if (strcmpa((CHAR8 *)"linux", key) == 0) {
			...
			entry->file = stra_to_path(value+1);
            entry->key = 'l';
            continue;
        }
        ...
        if (strcmpa((CHAR8 *)"architecture", key) == 0) {
            /* do not add an entry for an EFI image of architecture not matching with that of the brzboot image */
	...
	if (config_entry_check_validity( entry, device, initrd, file )) {
    	config_add_entry(config, entry);
	}
}
...
static VOID config_load(Config *config, EFI_HANDLE *device, EFI_FILE *root_dir, CHAR16 *loaded_image_path) {
    ...
    len = file_read(root_dir, L"\\EFI\\brzboot\\loader.conf", 0, 0, &content);
    if (len > 0)
    ...
    len = file_read(root_dir, L"\\EFI\\brzboot\\entries.conf", 0, 0, &content);
    if (len > 0)
...
EFI_STATUS efi_main(EFI_HANDLE image, EFI_SYSTEM_TABLE *sys_table) {
    ...
    efivar_set(L"LoaderInfo", L"brzboot " VERSION, FALSE);
    ...
    for (;;) {
        ...
        if (menu) {
            ...
            /* default splash */
            if (EFI_ERROR(err))
                graphics_splash(root_dir, L"\\EFI\\brzboot\\splash.bmp", config.background);
        }
