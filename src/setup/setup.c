...
/* search for "#### LoaderInfo: brzboot 31 ####" string inside the binary */
...
static int status_binaries(const char *esp_path, uint8_t partition[16]) {
        ...
        r = enumerate_binaries(esp_path, "EFI/brzboot", NULL);
        if (r == 0)
                fprintf(stderr, "Brzboot not installed in ESP.\n");
        else if (r < 0)
                return r;
...
static int create_dirs(const char *esp_path) {
        ...
        r = mkdir_one(esp_path, "EFI/brzboot");
        if (r < 0)
                return r;
...
static int copy_one_file(const char *esp_path, const char *name, bool force) {
        ...
        if (asprintf(&p, BRZBOOTLIBDIR "/%s", name) < 0) {
                fprintf(stderr, "Out of memory.\n");
                r = -ENOMEM;
                goto finish;
        }

        if (asprintf(&q, "%s/EFI/brzboot/%s", esp_path, name) < 0) {
                fprintf(stderr, "Out of memory.\n");
                r = -ENOMEM;
                goto finish;
        }
...
static int install_binaries(const char *esp_path, bool force) {
        ...
        d = opendir(BRZBOOTLIBDIR);
        if (!d) {
                fprintf(stderr, "Failed to open "BRZBOOTLIBDIR": %m\n");
                return -errno;
        }
...
static int find_slot(const uint8_t uuid[16], const char *path, uint16_t *id) {
        ...
        /* find already existing brzboot entry */
        for (i = 0; i < n_options; i++)
...
static int remove_binaries(const char *esp_path) {
        ...
        if (asprintf(&p, "%s/EFI/brzboot", esp_path) < 0) {
                fprintf(stderr, "Out of memory.\n");
                return -ENOMEM;
        }
        ...
        q = rmdir_one(esp_path, "EFI/brzboot");
        if (q < 0 && r == 0)
                r = q;
...
static int help(void) {
        printf("%s [COMMAND] [OPTIONS...]\n"
               "\n"
               "Install, update or remove the Brzboot EFI boot loader.\n\n"
               ...
               "\n"
               "Comands:\n"
               "     status          Show status of installed Brzboot and EFI variables\n"
               "     install         Install Brzboot to the ESP and EFI variables\n"
               "     update          Update Brzboot in the ESP and EFI variables\n"
               "     remove          Remove Brzboot from the ESP and EFI variables\n",
...
int main(int argc, char*argv[]) {
        ...
        case ACTION_UPDATE:
                ...
                if (arg_touch_variables)
                        r = install_variables(arg_path,
                                              ...
                                              "/EFI/brzboot/brzboot" MACHINE_TYPE_NAME ".efi",
        ...
        case ACTION_REMOVE:
                ...
                if (arg_touch_variables) {
                        q = remove_variables(uuid, "/EFI/brzboot/brzboot" MACHINE_TYPE_NAME ".efi", true);
                        if (q < 0 && r == 0)
                                r = q;
                }
                break;
